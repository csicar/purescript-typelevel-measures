...
import Type.Data.Peano.Int (class SumInt, N1, N2, N3, Neg, P1, P2, P3, P5, Pos)
...
sumOfInverted ::
  Array (Number : Ohm ()) ->
  Number
    : ( ampere :: MeasureExp AmpereT (Pos (Succ (Succ Z)))
    , kg :: MeasureExp KgT (Neg (Succ Z))
    , meter :: MeasureExp MeterT (Neg (Succ (Succ Z)))
    , sec :: MeasureExp SecT (Pos (Succ (Succ (Succ Z))))
    )
sumOfInverted rs = sum (liftV 0.0 // ohm) $ invertR rs

sumOfInverted' ::
  Array (Number : Ohm ()) -> Number : Ampere P2 * Kg N1 * Meter N2 * Sec P3 ()
sumOfInverted' rs =
  rs
    # map (\r -> liftV 1.0 // r)
    # foldl (++) (liftV 0.0 // ohm)
